...
if (class_exists('Niztech_Youtube') ) {
    include_once WP_PLUGIN_DIR . '/niztech-youtube/class-niztech-youtube-client.php';
}
...
if (! function_exists('hestia_child_eluminate_parent_css') ) {
    ...
    {
        wp_enqueue_style(
            'hestia_child_parent', trailingslashit(get_template_directory_uri()) . 'style.css',
            array( 'bootstrap' )
        );
        if (is_rtl() ) {
            wp_enqueue_style(
...
if (! function_exists('hestia_child_eluminate_load_google_analytics') ) {
    function hestia_child_eluminate_load_google_analytics()
    {
        wp_enqueue_script('ga-tag', 'https://www.googletagmanager.com/gtag/js?id=UA-134160689-1', null, null, true);
        wp_enqueue_script(
            'ga-include', get_stylesheet_directory_uri() . '/assets/js/google-analytics.js',
            [ 'ga-tag' ], null,
            true
        );
    }
